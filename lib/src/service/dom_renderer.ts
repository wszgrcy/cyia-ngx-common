...
export class DomRendererFactory2 implements RendererFactory2, OnDestroy {
  ...
  constructor(
    ...
    this.defaultRenderer = new DefaultDomRenderer2(eventManager, doc, ngZone, this.tracingService, this.nodeContext);
  }
...
class DefaultDomRenderer2 implements Renderer2 {
  ...
  constructor(
    ...
    private readonly nodeContext: ProxyNodeContext
  ) {}
  ...
  appendChild(parent: ContainerNode | ProxyNode, newChild: ChildNode): void {
    if (isProxyNode(parent)) {
      if (isProxyNode(newChild)) {
        newChild.linkProxy(parent);
      } else {
      ...
      if (isProxyNode(newChild)) {
        ...
        this.appendChildRaw(parent, newChild);
      }
    }
    this.nodeContext.appendChild(parent, newChild);
  }
  appendChildRaw(parent: any, newChild: any): void {
  ...
  removeChild(_parent: any, oldChild: ChildNode): void {
    let container;
  ...
  removeChildRaw(_parent: any, oldChild: any): void {
    // child was removed
    oldChild.remove();
  }

  selectRootElement(selectorOrNode: string | any, preserveContent?: boolean): any {
    let el: any = typeof selectorOrNode === 'string' ? this.doc.querySelector(selectorOrNode) : selectorOrNode;
    if (!el) {
  ...
  nextSibling(node: ChildNode): any {
    ...
    if (isProxyNode(maybeParent)) {
      ...
      list = this.nodeContext.boxMap.get(maybeParent as any)?.realList;
    }
    if (!list) {
      return null;
    }
  ...
  listen(
    ...
    return this.eventManager.addEventListener(target, event, wrappedCallback, options) as VoidFunction;
  }
...
class ShadowDomRenderer extends DefaultDomRenderer2 {
  ...
  constructor(
    ...
    nodeContext: ProxyNodeContext,

    private sharedStylesHost?: SharedStylesHost
  ) {
    super(eventManager, doc, ngZone, tracingService, nodeContext);
